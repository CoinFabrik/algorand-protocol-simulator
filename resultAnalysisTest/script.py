...
with open('src\out.log', 'r') as in_file:
    ...
    # print(list(stripped))
    r = re.compile(r"^\[INFO\]\tV.*")
    voteEvents = filter(r.match, stripped)
    voteEvents = [l[9:].split(" ") for l in voteEvents]
    
    with open("out.csv", "w", newline="") as f:
        writer = csv.writer(f)
        writer.writerow(["Node ID", "Account Address", "round", "period", "step", "Block hash value", "Vote Weight", "Simulation time (seconds)", "Chronological time (seconds)"])
        writer.writerows(voteEvents)